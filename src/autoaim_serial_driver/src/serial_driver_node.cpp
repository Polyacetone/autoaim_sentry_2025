...
private:
    ...
    std::shared_ptr<rclcpp::Publisher<autoaim_interfaces::msg::CompRobotsHp>>
    ...
    std::shared_ptr<rclcpp::Publisher<autoaim_interfaces::msg::RobotResource>>
    ...
    for (int i = 0; i < MAX_ATTEMPTS && rclcpp::ok(); i++) {
        try {
            if (!serial_driver_->port()->is_open()) {
                ...
                receive_thread_ = std::thread([&]() { receive_data(); });
                break;
            }
...
void SerialDriverNode::get_parameters() {
    ...
    rfr_robot_performance_pub_ = create_publisher<autoaim_interfaces::msg::RobotPerformance>(
        "/info/rfr_robot_performance",
        10
    );
    rfr_robot_pos_pub_ =
        create_publisher<autoaim_interfaces::msg::RobotPos>("/info/rfr_robot_pos", 10);
    rfr_robot_resource_pub_ =
    ...
    vel_sub_ = create_subscription<geometry_msgs::msg::Twist>(
        vel_sub_topic_,
        10,
        [&](const geometry_msgs::msg::Twist::SharedPtr msg) { send_vel(msg); }
    );
